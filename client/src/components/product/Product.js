...
const navigate = useNavigate()
  ...
  const getSingleProductDetail = async () => {
    ...
    const response = await axios.get(`${process.env.REACT_APP_BE_URL?process.env.REACT_APP_BE_URL: ""}/api/v1/products/${location.state.id}`);
    ...
    const getProducts1 = await axios.get(
      `${process.env.REACT_APP_BE_URL?process.env.REACT_APP_BE_URL: ""}/api/v1/product/?product_id=${product_id}`
    );
